...
import img1 from '../assets/menuitems/springroll.jpg';
...
export default function Menu(props) {
  ...
  // Initially open only the first category
  const [expanded, setExpanded] = useState(() =>
    menulist.reduce((acc, _, i) => {
      acc[i] = i === 0; // only first is open
      return acc;
    }, {})
  );
                ...
                {menulist.map((categories, key) =>
                    <div key={key} className='xl:w-[63rem]'>
                        <div id="category" className='flex justify-between items-center my-5 py-1 px-3 bg-gray-900'>
                            ...
                            <button onClick={()=>toggleCategory(key)} className='cursor-pointer'>
                        ...
                        <div className="itemlist flex flex-col justify-center items-center">
                            {categories.items.map((item, key) =>
                                <div key={key} id="item" className="flex pr-1 py-0 sm:pr-2 md:pr-3 md:py-0 my-3 justify-between max-w-[100%] lg:max-w-[820px] items-center h-[90px] sm:h-max border-gray-300 border-0 sm:border-2 bg-white">

                                    <div className="item-img w-[90px] h-[90px] sm:w-[110px] sm:h-[100px]">
